...
subprocess.run('docker build -t run_api_tests ./', shell=True, capture_output=True)
...
subprocess.run(f'docker run --rm --name run_robot_tests'
               ...
               f' run_api_tests:latest robot -d /opt/robotframework/results /opt/robotframework/tests',
